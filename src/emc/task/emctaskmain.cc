				...
				case EMC_TASK_MODE_MDI:	// ON, MDI
					switch (type) 
					{
						...
						default:
							...
							emcOperatorError(0, _("can't do that (%s:%d) in MDI mode"), emc_symbol_lookup(type),(int) type);
...
static int emcTaskIssueCommand(NMLmsg * cmd)
{
    ...
    if (emc_debug & EMC_DEBUG_TASK_ISSUE) 
	{
		rcs_print("Issuing %s -- \t (%s)\n", emcSymbolLookup(cmd->type), emcCommandBuffer->msg2str(cmd));
    }
    switch (cmd->type) 
	{
		// general commands
		case EMC_OPERATOR_ERROR_TYPE:
			retval = emcOperatorError(((EMC_OPERATOR_ERROR *) cmd)->id, "%s", ((EMC_OPERATOR_ERROR *) cmd)->error);
			break;

		case EMC_OPERATOR_TEXT_TYPE:
			retval = emcOperatorText(((EMC_OPERATOR_TEXT *) cmd)->id, "%s", ((EMC_OPERATOR_TEXT *) cmd)->text);
			break;

		case EMC_OPERATOR_DISPLAY_TYPE:
			retval = emcOperatorDisplay(((EMC_OPERATOR_DISPLAY *) cmd)->id, "%s", ((EMC_OPERATOR_DISPLAY *) cmd)->display);
			break;
		...
		case EMC_JOINT_SET_BACKLASH_TYPE:
			...
			retval = emcJointSetBacklash(set_backlash_msg->joint,
		...
		case EMC_AUX_INPUT_WAIT_TYPE:
			...
			if (emcAuxInputWaitMsg->timeout == WAIT_MODE_IMMEDIATE) 
